...
public class ManagementAccountResource {
    ...
    @PostMapping("/create")
    public ResponseEntity<Map<String, Object>> createAccount(@RequestBody ProfileDTO dto) {
        try {
            String tempPassword = accountManagementService.createEmployee(dto);
    ...
    public ResponseEntity<Map<String, Object>> searchAccount(@RequestBody EmailRequestDTO dto) {
        return accountManagementService.searchUserBasicInfo(dto.getEmail())
            .map(p -> {
                ...
                response.put("isActive", p.getIsActive());
